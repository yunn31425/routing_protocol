...
def send_udp_packet(cnt, destination_ip, destination_port, message):
    ...
    try:
        # Send the UDP packet
        udp_socket.sendto(message, dest_address)
...
if __name__ == "__main__":
    ...
    dest_port = 14568  # Change this to the appropriate destination port
    ...
    link_code_2 = 3
    reserved_2_2 = 0
    link_message_size_2 = 8
    neighbor_interface_address_2 = 0x44556677

    # Format the first OLSR packet header B_H_I
    olsr_header_1 = struct.pack("!HHBBHIBBH", packet_length_1, packet_sequence_number_1,\
    ...
    olsr_header_2 = struct.pack("!HBBBBHI", reserved_2, htime_2, willingness_2,\
    ...
    print("Combined Header (Hex):", combined_header)


    # Call the function to send the UDP packet
    while True:
        
        send_udp_packet(cnt, dest_ip, dest_port, combined_header)
        cnt = (cnt+1)%2
        sleep(0.3)
